...
@section('content')
    @if ($user)
        <title>{{ ucFirst($user) }}'s Profile</title>
        ...
        <div class='flex flex-col h-[80vh] lg:justify-center items-center mt-0'>
            ...
            <div class='text-sky-600 mt-2 w-[90%] lg:w-1/3 flex flex-row justify-around bg-zinc-700 py-5 '>
                <h2 class='inline font-medium text-2xl'>{{ $user }}</h2>
                <a href='/chats/{{ $user }}' class='text-white text-xl' title='Text User'><i
                        class="fa-solid fa-message"></i></a>
            </div>

            @if (strtolower(Cookie::get('username')) === strtolower($user))
